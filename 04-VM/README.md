...
- First, we created a kubernetes `Service` that is idential to the existing backend service with the exception of the workload selector targeting a different app label:
```yaml
selector:
  app: backend-vm
```
...
You'll also note a pretty simple `spec` definition that will inform the mesh how to connect to the envoy proxy (and visa versa) along with the labels that can be used to select our VM and associate with a other kubernetes objects, such as our `Service` we just defined.  The `network` attribute is also important as this instructs the mesh whether the VM and pods are directly routable or if there is a gateway required for communication.
```yaml
spec:
  address: $INTERNAL_IP
  labels:
...
5. At this point our service mesh and kubernetes cluster is configured and ready to go but our envoy proxy is not running or configured on our VM.  We'll use the sidecar-bootstrap command within the `tctl` cli to install the istio-proxy and join the VM to the mesh.  First, execute the command with the `--dry-run` flag so that you are able to see what is involved in th eprocess of bootstrapping.
...
7. Lastly, in order for us to route to this VM workload over the global mesh, we need to create a the listener on our `IngressGateway` that will broadcast globally how to discover and connect to the service.  Apply this configuration using the `tctl apply` command:
...
- Remove the security token and mesh configuration delivered to the VM via the boostrapping process found in `/etc/istio-proxy`:
```bash
rm /etc/istio-proxy/*
```

### Legacy app making request(s) to containerized apps
Now we will onboard the jumpbox VM as the *frontend* application.  We will utilze one of the backend container applications already deployed to the Global Service Mesh to test routing and service discovery from the VM.  The process will be very similar to onboarding as a backend.
...
Feel free to inspect the config we just applied in the file `02-frontend-vm.yaml`.  However, you'll note this is nearly idenitcal to the backend with the exception of object names and workload selectors.
